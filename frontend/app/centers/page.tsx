...
export default function CentersPage() {
  ...
  const stats = {
    ...
    critical: centers.filter((c: any) => c.status === 'critical').length,
    active: centers.filter((c: any) => c.status === 'active').length,
    stable: centers.filter((c: any) => c.status === 'stable').length,
    totalPeople: centers.reduce((sum: number, c: any) => sum + c.peopleCount, 0)
  }
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {filteredAndSortedCenters.map((center: any) => (
                        <div
                          key={center.id}
                          className={cn(

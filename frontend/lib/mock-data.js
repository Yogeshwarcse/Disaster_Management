// Mock data for Disaster Relief Resource Management System

import { calculatePriorityScore } from './algorithms'

// Central warehouse location (San Francisco)
...
// Relief Centers Data (Bay Area locations)
const centersData = [
  {
    id: 'center-001',
...
// Generate usage history data for charts
export function generateUsageHistory(days = 7) {
  const history = []
  const now = new Date()
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    history.push({
      ...
      water: Math.floor(Math.random() * 500) + 200,
      food: Math.floor(Math.random() * 300) + 100,
      medicine: Math.floor(Math.random() * 100) + 20,
      shelter: Math.floor(Math.random() * 50) + 10
    })
  }
...
export function generateDispatchStats() {
  return [
    { name: 'Delivered', value: 45, fill: 'var(--chart-2)' },
    { name: 'In Transit', value: 12, fill: 'var(--chart-1)' },
    { name: 'Pending', value: 8, fill: 'var(--chart-4)' },
    { name: 'Cancelled', value: 3, fill: 'var(--chart-5)' }
  ]
}

// Generate demand vs supply data
export function generateDemandSupplyData() {
  return initialInventory.map(item => {
    const totalDemand = initialCenters.reduce((sum, center) => {

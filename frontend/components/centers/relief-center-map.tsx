...
function MapController({ selectedCenter, centers }) {
  const map = useMap()
  ...
  useEffect(() => {
    if (selectedCenter) {
      ...
      const bounds = L.latLngBounds(centers.map(c => [c.location.lat, c.location.lng]))
...
function getPriorityColor(priorityScore, maxScore) {
...
function getRadius(peopleCount, maxPeople) {
  const minRadius = 15
  const maxRadius = 40
...
export function ReliefCenterMap({ 
  ...
  const maxPriorityScore = Math.max(...centers.map(c => c.priorityScore), 1)
  const maxPeopleCount = Math.max(...centers.map(c => c.peopleCount), 1)
